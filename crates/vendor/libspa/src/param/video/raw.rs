...
impl VideoInfoRaw {
    pub fn new() -> Self {
        // All-zero init is equivalent to upstream's field-by-field literal
        // (every value there is zero) but stays source-compatible with both
        // pre- and post-0.3.65 PipeWire headers, whose field sets differ.
    ...
    // `spa_video_info_raw.flags` only exists in PipeWire >= 0.3.65 headers;
    // these accessors are stubbed so the crate builds against Ubuntu 22.04's
    // PipeWire 0.3.48. Nothing in this workspace reads or writes video flags.
    ...
    pub fn set_modifier(&mut self, modifier: u64) {
        // Cast: the field is i64 in pre-0.3.65 headers and u64 afterwards.
